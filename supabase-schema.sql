...
-- Required when the table is created from SQL Editor rather than the
-- Supabase Table Editor. The browser uses the anon role through PostgREST.
grant select, insert, update, delete on table public.contacts to anon;
grant usage, select on sequence public.contacts_id_seq to anon;
