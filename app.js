// ---------------------------------------------------------------------------
// UHURU STORAGE / STARTUP
// Uses Supabase REST directly so the app does NOT depend on the Supabase CDN
// library loading. GitHub Pages can therefore still run the UI if that CDN is
// blocked or slow. Supabase remains the primary database; localStorage is the
...
const LOCAL_KEY = "UhuruContactBaseLocalV3";
...
function money(n) { n = Number(n) || 0; return "R " + n.toLocaleString("en-ZA"); }

function setConnection(state, message = "") {
  dbOnline = state === "online";
  dbError = message || "";
  const el = document.querySelector(".status");
  if (!el) return;
  el.classList.remove("online", "offline", "connecting");
  el.classList.add(state === "online" ? "online" : state === "offline" ? "offline" : "connecting");
  const label = state === "online" ? "Supabase connected" : state === "offline" ? "Offline / local mode" : "Connecting…";
  el.innerHTML = `<span></span>${esc(label)}`;
...
function showError(message) {
  ...
  dbError = text;
  setConnection("offline", text);
  alert("Uhuru Digital Solution\n\n" + text);
}
...
function localLoad() {
  ...
  } catch (e) { console.warn("Local data could not be read", e); return []; }
}
function localSave() {
  try { localStorage.setItem(LOCAL_KEY, JSON.stringify(contacts)); } catch (e) { console.warn("Local save failed", e); }
}
...
async function dbFetch(path, options = {}) {
  ...
  try {
    ...
    if (!res.ok) {
      const msg = body?.message || body?.hint || body?.details || (typeof body === "string" ? body : `${res.status} ${res.statusText}`);
      throw new Error(`Supabase ${res.status}: ${msg}`);
    }
    return body;
  } catch (e) {
    if (e.name === "AbortError") throw new Error("Supabase timed out after 10 seconds. Check the Supabase project/API settings.");
    throw e;
...
function toDb(c) {
  const row = {
    ...
    business: c.business,
    address: c.address || "Not specified",
    phone: c.phone,
    website: c.website || "",
    created: c.created || new Date().toISOString(),
    called: !!c.called,
    last_called: c.lastCalled || null,
    notes: c.notes || "",
    recall: !!c.recall,
    lead: !!c.lead,
...
function fromDb(r) {
  return {
    id: Number(r.id), number: r.number, business: r.business, address: r.address || "Not specified",
    phone: r.phone, website: r.website || "", created: r.created, called: !!r.called,
    lastCalled: r.last_called, notes: r.notes || "", recall: !!r.recall, lead: !!r.lead,
    status: r.status || "not-called", price: Number(r.price) || 0, paid: Number(r.paid) || 0,
    followUp: r.follow_up || "", siteUrl: r.site_url || ""
  };
}

async function openDB() {
  setConnection("connecting");
  await dbFetch("?select=id&limit=1", { method: "GET" });
  setConnection("online");
  return true;
}
async function all() {
  const data = await dbFetch("?select=*&order=number.asc,id.asc", { method: "GET" });
  return (data || []).map(fromDb);
}
async function put(c) {
  const data = await dbFetch("?select=*", {
    method: "POST",
...
async function update(c) {
  if (!dbOnline) { localSave(); return; }
  const id = encodeURIComponent(c.id);
  const row = toDb(c); delete row.id;
  await dbFetch(`?id=eq.${id}`, { method: "PATCH", headers: { Prefer: "return=minimal" }, body: JSON.stringify(row) });
}
async function del(id) {
  if (!dbOnline) { contacts = contacts.filter(c => c.id !== id); localSave(); return; }
  await dbFetch(`?id=eq.${encodeURIComponent(id)}`, { method: "DELETE" });
}
async function clearDB() {
  if (!dbOnline) { contacts = []; localSave(); return; }
  await dbFetch("?id=gt.0", { method: "DELETE" });
}
...
async function refresh() {
  try {
    ...
    console.warn("Using local cache because Supabase is unavailable:", err);
  }
  ...
  contacts.forEach((c, i) => {
    ...
    const st = statusOf(c);
    if (c.status !== st) { c.status = st; changed = true; }
  });
  localSave();
  if (changed && dbOnline) {
    for (const c of contacts) { try { await update(c); } catch (e) { console.warn("Normalisation sync failed", e); } }
  }
  render();
}

async function saveRecord(rec, isUpdate = false) {
  if (dbOnline) {
    try {
      if (isUpdate) await update(rec); else rec.id = await put(rec);
      setConnection("online");
      return true;
    } catch (err) {
      setConnection("offline", err.message);
      console.warn("Supabase write failed; keeping local copy:", err);
    }
  }
  // Local fallback: give the record an ID that cannot collide with a remote serial.
  if (!isUpdate && !rec.id) rec.id = Date.now() + Math.floor(Math.random() * 1000);
  return false;
}
...
async function add(b, a, p, website = "", silentDuplicate = false, extra = {}) {
  ...
  const remoteSaved = await saveRecord(rec, false);
  if (!remoteSaved) {
    contacts.push(rec);
    contacts.sort((x, y) => (Number(x.number) || 0) - (Number(y.number) || 0));
    localSave();
    render();
    alert("Saved on this device. Supabase is currently unavailable, so it will sync when the connection works.");
    return true;
  }
  await refresh();
  return true;
...
window.addEventListener("error", e => console.error("Uhuru error:", e.error || e.message));
window.addEventListener("unhandledrejection", e => {
  console.error("Uhuru promise error:", e.reason);
  e.preventDefault();
  setConnection("offline", e.reason?.message || String(e.reason || "Unknown error"));
});

// Render and theme FIRST. Database connectivity must never block the UI.
try { render(); } catch (e) { console.error(e); }
openDB().then(refresh).catch(err => {
  contacts = localLoad();
  setConnection("offline", err.message);
  render();
});
