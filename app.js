...
/* ---------------------------------------------------------------
   ...
   ------------------------------------------------------------- */
/* ============================================================
   🔑 CONFIGURATION: Replace with YOUR actual Supabase keys
...
// Initialize Supabase client — checks if credentials are real
const supabase = (() => {
  const hasUrl = SUPABASE_URL.startsWith("https://") && !SUPABASE_URL.includes("YOUR-PROJECT");
  const hasKey = SUPABASE_ANON_KEY && SUPABASE_ANON_KEY.length > 20 && !SUPABASE_ANON_KEY.includes("YOUR-ANON");
...
The app is not connected to your database. Here's what to do:

1. Go to https://app.supabase.com and sign in to your project
2. Click Settings → API (left sidebar)
3. Copy your PROJECT URL (looks like: https://xxxxx.supabase.co)
4. Copy your ANON PUBLIC KEY (a long string)
5. Open app.js and replace these lines (near the top):
   const SUPABASE_URL = "YOUR-PROJECT-URL-HERE";
   const SUPABASE_ANON_KEY = "YOUR-KEY-HERE";
6. Save and refresh this page

Questions? See SUPABASE-SETUP.md for detailed steps.
      `.trim();
...
openDB().then(refresh).catch(err => {
  ...
  // Show error in the contacts area
  const box = document.querySelector("#contacts");
  if (box) {
  ...
  // Also alert (for GitHub Pages visibility)
  setTimeout(() => {
