...
// ---------------------------------------------------------------------------
// STORAGE LAYER (Supabase) — same function names/shapes the rest of the app
// already uses, so nothing below this needed to change.
// App-side objects stay camelCase; we map to/from the DB's snake_case columns.
// ---------------------------------------------------------------------------
function toDb(c) {
  const row = {
...
async function openDB() {
  // No local database to open anymore — just confirm we can reach Supabase.
...
async function put(c) {
  const { data, error } = await supabase.from(TABLE).insert(toDb(c)).select("id").single();
  if (error) throw error;
  return Number(data.id);
}

async function update(c) {
  const { id, ...rest } = toDb(c);
  const { error } = await supabase.from(TABLE).update(rest).eq("id", c.id);
  if (error) throw error;
}
