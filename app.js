// ---------------------------------------------------------------------------
// SUPABASE CONFIG + RESILIENT STARTUP
// ---------------------------------------------------------------------------
...
const LOCAL_KEY = "UhuruContactBaseLocalV4";
let supabase = null;
...
function showConnection(ok, message) {
  ...
  if (panel && msg) { panel.style.display = ok ? "none" : "block"; msg.textContent = ok ? "" : message; }
}
...
function initSupabase() {
  if (window.supabase && typeof window.supabase.createClient === "function") {
    try { supabase = window.supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY, { auth: { persistSession:false, autoRefreshToken:false } }); return true; }
    catch(e) { connectionError=e?.message||String(e); }
  } else connectionError="Supabase library did not load.";
  return false;
}
initSupabase();
let contacts = [];
...
// ---------------------------------------------------------------------------
// STORAGE LAYER — Supabase first, local fallback when unavailable.
// ---------------------------------------------------------------------------
function toDb(c) {
  const row = { number:c.number??null, business:c.business, address:c.address||"Not specified", phone:c.phone, website:c.website||"", created:c.created||new Date().toISOString(), called:!!c.called, last_called:c.lastCalled||null, notes:c.notes||"", recall:!!c.recall, lead:!!c.lead, status:c.status||"not-called", price:Number(c.price)||0, paid:Number(c.paid)||0, follow_up:c.followUp||null, site_url:c.siteUrl||"" };
  if(c.id!=null) row.id=c.id; return row;
}
function fromDb(r) {
  return { id:Number(r.id), number:r.number, business:r.business, address:r.address||"Not specified", phone:r.phone, website:r.website||"", created:r.created, called:!!r.called, lastCalled:r.last_called, notes:r.notes||"", recall:!!r.recall, lead:!!r.lead, status:r.status||"not-called", price:Number(r.price)||0, paid:Number(r.paid)||0, followUp:r.follow_up||"", siteUrl:r.site_url||"" };
}
async function probeSupabase() {
  if(!supabase) throw new Error(connectionError||"Supabase is not available.");
  const {error}=await supabase.from(TABLE).select("id",{count:"exact",head:true}); if(error) throw error;
}
async function openDB() {
  try {
    // Give a CDN fallback script a moment to load on GitHub Pages.
    if (!supabase) {
      for (let i = 0; i < 15 && !window.supabase; i++) await new Promise(r => setTimeout(r, 200));
      initSupabase();
    }
    await probeSupabase();
    supabaseReady = true;
    showConnection(true,"Supabase connected");
    // Push contacts that were created while the database was unavailable.
    const local = localLoad();
    if (local.length) {
      const { data } = await supabase.from(TABLE).select("phone");
      const existing = new Set((data || []).map(r => phoneKey(r.phone)));
      const remaining = [];
      for (const c of local) {
        if (existing.has(phoneKey(c.phone))) continue;
        const copy = { ...c }; delete copy.id;
        const { error } = await supabase.from(TABLE).insert(toDb(copy));
        if (error) remaining.push(c);
      }
      if (remaining.length) localSave(remaining); else localStorage.removeItem(LOCAL_KEY);
    }
  } catch(e) {
    supabaseReady=false;
    connectionError=e?.message||String(e);
    showConnection(false,"Supabase unavailable: "+connectionError+" — local saving is enabled.");
  }
}
async function all() {
  if(supabaseReady) { const {data,error}=await supabase.from(TABLE).select("*").order("number",{ascending:true}); if(!error) return (data||[]).map(fromDb); supabaseReady=false; connectionError=error.message||String(error); showConnection(false,"Supabase read failed: "+connectionError+" — local saving is enabled."); }
  return localLoad();
}
async function put(c) {
  if(supabaseReady) { const {data,error}=await supabase.from(TABLE).insert(toDb(c)).select("id").single(); if(!error&&data) return Number(data.id); supabaseReady=false; connectionError=error?.message||"Supabase insert failed."; showConnection(false,"Supabase save failed: "+connectionError+" — contact saved locally."); }
  const list=localLoad(), id=Date.now()+Math.floor(Math.random()*1000); list.push({...c,id}); localSave(list); return id;
}
async function update(c) {
  if(supabaseReady) { const {id,...rest}=toDb(c); const {error}=await supabase.from(TABLE).update(rest).eq("id",c.id); if(!error) return; supabaseReady=false; connectionError=error.message||"Supabase update failed."; showConnection(false,"Supabase update failed: "+connectionError+" — changes saved locally."); }
  localSave(localLoad().map(x=>Number(x.id)===Number(c.id)?{...x,...c}:x));
}
async function del(id) {
  if(supabaseReady) { const {error}=await supabase.from(TABLE).delete().eq("id",id); if(!error) return; supabaseReady=false; connectionError=error.message||"Supabase delete failed."; showConnection(false,"Supabase delete failed: "+connectionError+" — local copy updated."); }
  localSave(localLoad().filter(x=>Number(x.id)!==Number(id)));
}
async function clearDB() {
  if(supabaseReady) { const {error}=await supabase.from(TABLE).delete().gt("id",0); if(!error) return; supabaseReady=false; connectionError=error.message||"Supabase clear failed."; showConnection(false,"Supabase clear failed: "+connectionError); }
  localSave([]);
}
...
async function add(b, a, p, website = "", silentDuplicate = false, extra = {}) {
  ...
  p = cleanPhone(p);
  if (!b || !p) {
    alert("Business name and contact number are required.");
    return false;
  }
...
document.querySelector("#clearBtn").onclick = async () => {
  if (confirm("Delete ALL contacts from Supabase/local storage? This cannot be undone.")) {
    await clearDB();
    await refresh();
  }
};
...
window.addEventListener("unhandledrejection", e => {
  ...
  alert("Sync error: " + (e.reason?.message || e.reason || "could not reach the database. Check your internet connection."));
  e.preventDefault();
});

openDB().then(refresh).catch(err => alert("Could not connect to Supabase: " + err.message));
...
document.querySelector("#retryBtn")?.addEventListener("click", async () => {
  showConnection(false,"Retrying Supabase connection…");
  await openDB();
  await refresh();
});
